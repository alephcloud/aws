...
normalizeUriQuery =
    -- normalize case and percent encoding
    HTTP.parseQueryText . BB.toByteString . HTTP.renderQueryText True
...
canonicalHeaders = CanonicalHeaders
    ...
    --
    -- TODO maybe we make this a compile time flag are an parameter?
    --
    -- . map canonicalDate
  where
    ...
    canonicalDate ("date", d) = ("date", formatDate d)
      where
        formatDate :: B8.ByteString -> B8.ByteString
        formatDate d = case parseHttpDate (B8.unpack d) of
            Nothing -> d
...
signingKey credentials s = SigningKey kSigning
  where
    kSecret = secretAccessKey credentials
...
authorizationCredential creds credScope =
    (fromString . B8.unpack . accessKeyID) creds <> "/" <> toText credScope
...
--
-- TODO: is it possible to use the @authorization@ header and still place the
-- query into the URI? This may be required for some services that require
-- a different content type (for example @application/json@).
--
-- == GET Request
--
...
--
-- Somewhat surprisingly (and cover neither by the AWS Signature V4 test suite
...
getSigningKey credentials region service = do
    ...
    k <- atomicModifyIORef' (v4SigningKeys credentials) $ \cache ->
